...
public class FillFormHelper extends HelperBase {
    ...
    public void selectRadioBtn(String rbtnValue) throws RuntimeException {
        if (Objects.equals(rbtnValue, "Вариант 2.1")) {
            ...
            throw new RuntimeException("Указано неправильное имя радиокнопки");
        }
    }

    @Step("Выбор чекбокса")
    public void selectCheckbox(String chbValue) throws RuntimeException {
        if (Objects.equals(chbValue, "Вариант 1.1")) {
            ...
            throw new RuntimeException("Указано неправильное имя чекбоска");
        }
    }
    ...
    Map<String, String> row = new LinkedHashMap<>();
        row.put("E-Mail", emailExp);
        row.put("Имя", nameExp);
        row.put("Пол", genderExp);
        row.put("Выбор 1", select1Exp);
        row.put("Выбор 2", select2Exp);
        expectedTableData.add(row);
    ...
    public List<Map<String, String>> getTableData(String tableXPath) {
        ...
        //Нахождение строк (Rows)
        List<WebElement> rows = driver.findElements(By.xpath(tableXPath + "//tbody//tr"));
        for (WebElement row : rows) {
            Map<String, String> rowMap = new LinkedHashMap<>();
            //Нахождение ячеек в текущей строке
            List<WebElement> cells = row.findElements(By.tagName("td"));

            for (int i = 0; i < headers.size(); i++) {
                //Заполнение Map: ключ = заголовок, значение = текст ячейки
    ...
    @Step("Поиск текста в таблице по номеру строки")
    public String getCellText(int rowIndex, int columnIndex) {
        String xpath = String.format("//table[@id='dataTable']/tbody/tr[%d]/td[%d]", rowIndex, columnIndex);
